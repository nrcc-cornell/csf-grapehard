...
class DisplaySeason extends Component {
  ...
  render() {
        ...
        if ( (this.props.store.app.seasonStatus) && (this.props.store.app.getChartData) ) {
            ...
            const getXaxisMin = () => {
                return Date.UTC(firstYearOfSeason,8,15)
            }

            const getXaxisMax = () => {
                return Date.UTC(secondYearOfSeason,5,30)
            }
            ...
            const afterRender = (chart) => { chart.renderer.text(firstYearOfSeason+'-'+secondYearOfSeason+' Season To Date', 300, 85).css({ color:"#000000", fontSize:"16px"}).add() };
            ...
            var chartConfig = {
                 ...
                 xAxis: { type: 'datetime', startOnTick: true, endOnTick: true, min: getXaxisMin(), max: getXaxisMax(), labels: { align: 'center', x: 0, y: 20 },
                 ...
                 series: [{
                     ...
                     name: 'Hardiness Temp', data: data[variety]['hardtemp'].slice(0,idxFirstFcst), color: '#00bb00', lineWidth: 2, marker: { enabled: true, radius: 3 }, zIndex: 18 },{
                     name: 'Hardiness Forecast', pointStart: firstFcstDate, data: data[variety]['hardtemp'].slice(idxFirstFcst), dashStyle: 'dot', color: '#00bb00', lineWidth: 2, marker: { enabled: true, radius: 3 }, showInLegend: fcstInView() },{
                     ...
                     name: 'Min Temp Forecast', pointStart: firstFcstDate, data: data[variety]['mint'].slice(idxFirstFcst),  dashStyle: 'dot', color: '#0000FF', step: false, lineWidth: 2, marker: { enabled: true, radius: 3 }, showInLegend: fcstInView() }
                 ]
            };
